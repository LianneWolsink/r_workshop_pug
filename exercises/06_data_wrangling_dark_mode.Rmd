...
```{r}
nerd_red <- nerd %>% select(age, gender, married, voted, nerdy, Q1:Q26)
```
...
```{r}
nerd_red %>%  filter (gender==2 & age<30)
```
...
```{r}
nerd_red %>% filter(age==22 | age ==26)
```
2.3) ... anyone who is either a man (`gender == 1`) **or** older than 30.
```{r}
nerd_red %>% filter(gender==1 | age>30)
```
...
```{r}
nerd_red %>% filter(gender==3 & (married==2 | married==3))
# or:
# nerd_red %>% filter(gender==3 & married!=1)
```
...
```{r}
nerd_red %>% filter(gender==2 & nerdy==7) %>% nrow()
nerd_red %>% filter(gender==2 & nerdy==1) %>% nrow()
nerd_red %>% filter(gender==1 & nerdy==7) %>% nrow()
nerd_red %>% filter(gender==1 & nerdy==1) %>% nrow()
```
## Summaries
...
```{r}
nerd_red_test <- nerd_red %>% filter(age>18 & age<100 & nerdy !=0 & married !=0 & voted !=0)
# However, there might be values other than 1:3 that are still not valid
# So:
nerd_red_test <- nerd_red %>% filter(age>=18 & age<=100 & nerdy !=0 & married !=0 & voted !=0)
nerd_red <- nerd_red %>% filter(age>=18, age<=100, married %in% 1:3, voted %in% 1:2, nerdy %in% 1:7)

```
4.3) Confirm that the filtering worked by running checks on `nerdy`, `married`, `voted` and `age`.
```{r}
nerd_red %>% count(nerdy)
nerd_red %>% count(married)
nerd_red %>% count(voted)
nerd_red %>% count(age)
# or
...
```{r}
nerd_red %>% mutate(voted = ifelse(voted==1, "yes", "no"))
```
...
```{r}
nerd_red %>% mutate(sum_Q1_Q2 = Q1+Q2)
```
