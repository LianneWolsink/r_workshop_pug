...
```{r}
nerd_red[1:50, ] %>% # first 50 rows, all colunmns
...
nerd_red %>% 
  ...
  geom_point() +
  labs(y = "sum score nerdiness questionnaire", 
       title = "Relation between age and nerd score", 
...
nerd_red %>% 
  ggplot(aes(x = nerd_score, y = age)) +
  geom_point() +
  scale_x_continuous(breaks = seq(0, 200, 10), limits = c(10,200))
```
...
nerd_red %>% 
  ggplot(aes(x = age, y = nerd_score)) +
  geom_point(size = 4, alpha = 0.5, colour = "#ff449d")
```
